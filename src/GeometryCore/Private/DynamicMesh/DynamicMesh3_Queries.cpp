...
FAxisAlignedBox3d FDynamicMesh3::GetBoundsForVertexSelection(TConstArrayView<int32> VertexIDs) const
{
	...
	constexpr int32 ChunkSize = 4096*2;
	constexpr int32 MinCountToThread = ChunkSize * 3;
	
	constexpr bool bParallel = true;
	if (!bParallel || NumVertices < MinCountToThread)
	{
	...
	{
		int32 NumChunks = (NumVertices / ChunkSize) + 1;
		FCriticalSection BoundsLock;
		ParallelFor(NumChunks, [&](int ci)
		{
			FAxisAlignedBox3d ChunkBounds = FAxisAlignedBox3d::Empty();
			int32 Start = ci * ChunkSize;
			int32 End = FMath::Min(Start + ChunkSize, NumVertices);
...
FAxisAlignedBox3d FDynamicMesh3::GetBoundsForTriangleSelection(TConstArrayView<int32> TriangleIDs) const
{
	...
	constexpr bool bParallel = true;
	if (!bParallel || NumTriangles < MinCountToThread)
	{
		UpdateBoundsWithTriangles(ResultBounds, TriangleIDs);
	}
	else
	{
		int32 NumChunks = (NumTriangles / ChunkSize) + 1;
		FCriticalSection BoundsLock;
		ParallelFor(NumChunks, [&](int ci)
		{
			FAxisAlignedBox3d ChunkBounds = FAxisAlignedBox3d::Empty();
			int32 Start = ci * ChunkSize;
			int32 End = FMath::Min(TriangleIDs.Num(), Start + ChunkSize);
			TConstArrayView<int32> IndexArrayView(TriangleIDs.GetData() + Start, End - Start);
			UpdateBoundsWithTriangles(ChunkBounds, IndexArrayView);
			BoundsLock.Lock();
			ResultBounds.Contain(ChunkBounds);
			BoundsLock.Unlock();
		});
...
{
	const FIndex3i& Triangle = Triangles[tID];
	return VectorUtil::TriangleInternalAngles(Vertices[Triangle[0]], Vertices[Triangle[1]], Vertices[Triangle[2]]);
}
