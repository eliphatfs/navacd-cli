...
namespace VectorUtil
{
	...
	/** @return true if Abs(A-B) is less or equal than Epsilon */
	template <typename RealType>
	...
	*/
	...
	/**
	 * @return angle in degrees between vectors (A-CornerPt) and (B-CornerPt)
	 */
	template<typename RealType>
	inline RealType OpeningAngleD(const TVector<RealType>& A, const TVector<RealType>& B, const TVector<RealType>& P)
	{
		return AngleD(Normalized(A - P), Normalized(B - P));
	}

	/**
	 * @return angle in radians between vectors (A-CornerPt) and (B-CornerPt)
	 */
	template<typename RealType>
	inline RealType OpeningAngleR(const TVector<RealType>& A, const TVector<RealType>& B, const TVector<RealType>& P)
	{
		return AngleR(Normalized(A - P), Normalized(B - P));
	}

	/**
	 * @return the internal angles of triangle ABC
	 */
	template<typename RealType>
	inline TVector<RealType> TriangleInternalAngles(const TVector<RealType> A, const TVector<RealType> B, const TVector<RealType>& C)
	{
	...
	/** 
	 * @return oriented dihedral angle of two triangles sharing an oriented edge
	 * 
	 * @note Provided normals and Edge should be unit length, and Edge be perpendicular to both normals 
	 * (see Keenan Crane, Discrete Differential Geometry: An Applied Introduction, Appendix 1.5) 
	 */
	template <typename RealType>
