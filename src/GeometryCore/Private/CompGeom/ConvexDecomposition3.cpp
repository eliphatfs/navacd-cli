...
bool FConvexDecomposition3::ConvexPartVsSphereOverlap(const FConvexDecomposition3::FConvexPart& Part, FVector3d Center, double Radius, const FTransform* TransformIntoSphereSpace, double* OutDistanceSq)
{
	...
	for (int32 PlaneIdx = 0; PlaneIdx < Part.HullPlanes.Num(); ++PlaneIdx)
	{
		...
		if (bMustTransformConvex)
		{
			Plane.Transform(*TransformIntoSphereSpace); //-V522
		}
...
bool FConvexDecomposition3::AddCoveringToInitialNegativeSpace(const FSphereCovering& Covering, FMatrix CoveringToLocalSpace, double RadiusToLocalScale)
{
	if (Decomposition.Num() != 1)
	{
		return false;
	}
	int32 OrigNum = NegativeSpace.Num();
	RadiusToLocalScale /= FMath::Max(FMathd::ZeroTolerance, ResultTransform.GetScale3D().GetAbsMin());
	CoveringToLocalSpace *= ((FTransform)ResultTransform).ToInverseMatrixWithScale();
	for (int32 SphereIdx = 0, Num = Covering.Num(); SphereIdx < Num; ++SphereIdx)
	{
		FVector3d Center = CoveringToLocalSpace.TransformPosition(Covering.GetCenter(SphereIdx));
		double Radius = Covering.GetRadius(SphereIdx) * RadiusToLocalScale;
		NegativeSpace.AddSphere(Center, Radius);
	}
	InitNegativeSpaceConvexPartMapping(OrigNum);
	return true;
}
...
void FConvexDecomposition3::InitNegativeSpaceConvexPartMapping(int32 StartSphereIndex)
{
	// Helper to check each sphere for overlap vs each convex part
	auto IteratePartsVsSpheres = [this, StartSphereIndex](TFunctionRef<void(FConvexPart& Part, int32 SphereIdx, bool bOverlapsPart)> ProcessPartVsSphere)
	{
		for (int32 PartIdx = 0; PartIdx < Decomposition.Num(); ++PartIdx)
		{
			FConvexPart& Part = Decomposition[PartIdx];
			if (StartSphereIndex == 0 && !ensure(Part.OverlapsNegativeSpace.IsEmpty())) // If calling on all spheres, expect this Init to only be called when there is no existing mapping
			{
				Part.OverlapsNegativeSpace.Reset();
			}
			else if (StartSphereIndex > 0 && !Part.OverlapsNegativeSpace.IsEmpty()) // If calling on subset of spheres, expect this Init to be called only a larger StartSphereIndex than previously mapped spheres
			{
				// Spot-check just the last sphere to catch out incorrect usage, since spheres are generally added in increasing order
				ensure(Part.OverlapsNegativeSpace.Last() < StartSphereIndex);
			}
			for (int32 SphereIdx = StartSphereIndex; SphereIdx < NegativeSpace.Num(); ++SphereIdx)
			{
