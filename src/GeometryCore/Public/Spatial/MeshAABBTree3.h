		...
		int& NearestVertexID, const FQueryOptions& Options)
	{
		...
		{ // triangle-list case, array is [N t1 t2 ... tN]
			...
			for (int i = 1; i <= num_tris; ++i)
			{
				...
				for (int j = 0; j < 3; ++j) 
				{
					double VertexDistSqr = DistanceSquared((FVector3d)Mesh->GetVertex(Triangle[j]), P);
	...
	) const
	{
		...
		for (int TID = 0, N = TestMesh->MaxTriangleID(); TID < N; TID++)
		{
			if (TransformF != nullptr)
			{
				...
				TestTri.V[0] = TransformF((FVector3d)TestMesh->GetVertex(Tri.A));
				TestTri.V[1] = TransformF((FVector3d)TestMesh->GetVertex(Tri.B));
				TestTri.V[2] = TransformF((FVector3d)TestMesh->GetVertex(Tri.C));
			}
			else
			{
