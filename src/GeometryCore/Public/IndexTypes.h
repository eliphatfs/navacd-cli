...
// NavACD standalone: FIndex2i/FIndex3i/FIndex4i are provided by our shim Types.h
// in the global namespace. We skip the UE::Geometry definitions here.
#ifndef NAVACD_INDEX_TYPES_PROVIDED
...
inline uint32 GetTypeHash(const FIndex2i& Index)
{
...
inline uint32 GetTypeHash(const FIndex3i& Index)
{
	// (this is how FIntVector and all the other FVectors do their hash functions)
...
inline uint32 GetTypeHash(const FIndex4i& Index)
{
	// (this is how FIntVector and all the other FVectors do their hash functions)
	// Note: this assumes there's no padding that could contain uncompared data.
...
#endif // NAVACD_INDEX_TYPES_PROVIDED
