...
struct FNegativeSpaceSampleSettings
{
	...
	// Optional function to define a bias (scalar multiplier) for the ReduceRadiusMargin value, to support scaling the tolerance based on location or direction to surfaces.
	// The custom method takes the reference mesh, nearest triangle ID on that mesh, and the query position,
	// and returns a scale factor between 0 and 1 to apply to the ReduceRadiusMargin
	TFunction<double(const FDynamicMesh3& Mesh, int32 NearTID, FVector3d QueryPos)> CustomRadiusMarginScale;

	// Margin scales should be between this value and 1.0. When the MinCustomMarginScale is 1.0, no custom scaling is applied.
	// Should be greater than 0. Note that the sampling will be more expensive to compute for smaller minimum scale values.
	...
	void Sanitize()
	{
		...
		MinCustomMarginScale = FMath::Clamp(MinCustomMarginScale, .1, 1.0);
	}
...
public:
	...
	// Apply transform to spheres
	// Note: Non-uniform scales will scale the radius by the smallest scale axis's magnitude (matching how physics collision transforms spheres)
	void ApplyTransform(const FTransform& Transform)
	{
		TransformRange(0, Position.Num(), Transform);
	}

	// Append spheres from Other w/ the given Transform
	// Note: Non-uniform scales will scale the radius by the smallest scale axis's magnitude (matching how physics collision transforms spheres)
	void AppendTransformed(const FSphereCovering& Other, const FTransform& Transform)
	{
		int32 AppendStart = Position.Num();
		Position.Append(Other.Position);
		Radius.Append(Other.Radius);
		TransformRange(AppendStart, Position.Num(), Transform);
	}
...
private:
	...
	void TransformRange(const int32 Start, const int32 Max, const FTransform& Transform)
	{
		for (int32 Idx = Start; Idx < Max; ++Idx)
		{
			Position[Idx] = Transform.TransformPosition(Position[Idx]);
		}
		double RadiusScale = Transform.GetScale3D().GetAbsMin();
		for (int32 Idx = Start; Idx < Max; ++Idx)
		{
			Radius[Idx] *= RadiusScale;
		}
	}
};
...
public:
	...
	struct FPreprocessMeshOptions
	{
		...
		// Note: If passed to the FConvexDecomposition3 constructor, will also set the similar ThickenAfterHullFailure 
		// member, which applies to generated parts of the decomposition.
		double ThickenInputAfterHullFailure = 0.0;
	...
	{
		ThickenAfterHullFailure = Options.ThickenInputAfterHullFailure;
	 ...
	 * @param Vertices			Vertex buffer for mesh to decompose
	 * @param Faces				Triangle buffer for mesh to decompose
	 * @param Options			Options for pre-processing the mesh before decomposition (e.g., to weld matching edges, thicken degenerate inputs, or to run a general pre-process lambda function)
	 * @param FaceVertexOffset	Indices from the Faces array are optionally offset by this value.  Useful e.g. to take slices of the multi-geometry vertex and face buffers of FGeometryCollection.
	 */
	 ...
	 * @param RequestedSamples Optional points where we should sample the negative space, in addition to spheres automatically added via the Settings. Settings still apply to filter (bAllowSamplesInsideMesh) and to determine sphere radii.
	 ...
	 * @param Covering The spheres to add.
	 * @param CoveringToLocalSpace Matrix transform from input covering space to the local space of input convex decomposition geometry
	 * @param RadiusToLocalScale Scale to transform sphere radii to the local space of the convex decomposition. Note spheres cannot be non-uniformly scaled in the local space.
	 */
	GEOMETRYCORE_API bool AddCoveringToInitialNegativeSpace(const FSphereCovering& Covering, FMatrix CoveringToLocalSpace = FMatrix::Identity(), double RadiusToLocalScale = 1.);
...
private:
	...
	void InitNegativeSpaceConvexPartMapping(int32 StartSphereIndex = 0);
