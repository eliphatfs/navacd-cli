...
{
	if (copy.IsCompact() && ((!bAttributes || !copy.HasAttributes()) || copy.Attributes()->IsCompact())) {
...
void UE::Geometry::Debug::ClearAllDebugMeshes()
{
	...
	{
		UE_LOGF(LogGeometry, Warning, "ClearAllDebugMeshes() called but geometry.DynamicMesh.EnableDebugMeshes CVar is disabled");
		return;
	}

	UELocal::GlobalDebugMeshes.Reset();
#else
	UE_LOGF(LogGeometry, Warning, "DynamicMesh3 Global Debug Mesh support is only available in-Editor");
#endif
}
...
void UE::Geometry::Debug::StashDebugMesh(const FDynamicMesh3& Mesh, FString DebugMeshName)
{
	...
	if (CVarDynamicMeshDebugMeshesEnabled.GetValueOnAnyThread() == false )
	{
		UE_LOGF(LogGeometry, Warning, "StashDebugMesh() called but geometry.DynamicMesh.EnableDebugMeshes CVar is disabled")
		return;
	}
...
bool UE::Geometry::Debug::FetchDebugMesh(FString DebugMeshName, FDynamicMesh3& MeshOut, bool bClear)
{
	...
	if (CVarDynamicMeshDebugMeshesEnabled.GetValueOnAnyThread() == false )
	{
		UE_LOGF(LogGeometry, Warning, "FetchDebugMesh() called but geometry.DynamicMesh.EnableDebugMeshes CVar is disabled");
		return false;
	}
